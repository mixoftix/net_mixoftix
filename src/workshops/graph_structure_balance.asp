...
<head>
    ...
    <title>Graph Structure (Balance Model)</title>
...
<body class="dark-mode">
    ...
    <header>
        <h1>Graph Structure (Balance Model)</h1>
    ...
    <section class="content-box">
        ...
        <a href="/workshops/block_structure_utxo.asp">Graph Structure (Balance Model)</a>
    </section>

    <main>
        <section class="content-box">
            <h2>Alice to Bob Transaction (Simple Fork Scenario)</h2>
            <p>Change Alice's transaction amount per graph. Each graph updates independently.</p>
        </section>
...
<script>
    ...
    function createGraphs() {
        ...
        for (let g = 1; g <= NUM_GRAPHS; g++) {
            ...
            let html = `
                <tr>
                    <th colspan="3" class="miner-header">Graph ${g} (Transaction 17)</th>
                </tr>
                <tr>
                ...
                <tr>
                    <td>
                        <div class="input-container">
                            <input id="aliceRaw_${g}" value="0000000" maxlength="7" data-graph="${g}">
                        ...
                        <div class="timestamp" id="timestamp_${g}">Timestamp: 0</div>
                    </td>
    ...
    function calculateGraph(graphNum) {
        const rawInput = document.getElementById(`aliceRaw_${graphNum}`);
        const rawVal = binaryToDecimal(rawInput.value);
        const decimalEl = document.getElementById(`decimal_${graphNum}`);

        decimalEl.textContent = `(${rawVal})`;

        // Timestamp
        const tsEl = document.getElementById(`timestamp_${graphNum}`);
        tsEl.textContent = `Timestamp: ${getUnixTimestamp()}`;
        tsEl.classList.remove('timestamp-highlight');
        void tsEl.offsetWidth;
        tsEl.classList.add('timestamp-highlight');

        const aliceCell = document.getElementById(`aliceNew_${graphNum}`);
        const bobCell = document.getElementById(`bobNew_${graphNum}`);
    ...
    function highlightMatchingLastDigits() {
        const cells = document.querySelectorAll('#graphsContainer .value-cell strong');
        const digitMap = {};
